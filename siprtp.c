...
/* 
 * Copyright (C) 2009 Paul Boyle paulcboyle@gmail.com 
				   ...
				   pjsip_event *e)
{
    ...
    myHeader.portNumber = call->serverChanID;
    myHeader.command = DELETE_CHANNEL;

    closeChannel(call->serverChanID,call->server);
...
//#warning "This needs to be fixed to handle arbitary codec."
  ...
  //  sdp->conn->addr = app.local_addr;
    sdp->conn->addr = pj_str(call->server->ip);
  ...
  //  m->desc.port = pj_ntohs(tpinfo.sock_info.rtp_addr_name.ipv4.sin_port);
    m->desc.port = myHeader.portNumber ;//pj_ntohs(tpinfo.sock_info.rtp_addr_name.ipv4.sin_port);
    m->desc.port_count = 1;
